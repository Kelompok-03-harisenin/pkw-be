...
const uploadPhoto = async (req, res, next) => {
  try {
    ...
    const result = await addPhotoService(userId, id_category, req.img, title, description);
    res
      .status(201)
      .send({ message: "Berhasil menambahkan foto", data: result });
  } catch (err) {
    console.log(err);
    next(err);
  }
  ...
  await PhotoModel.update(
    {
      ...
      photo_url: req.img || photo_url || existingPhoto.photo_url,
